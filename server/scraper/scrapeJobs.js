...
async function scrapeActuaryList(page, maxPages = 1) {
  ...
  for (let p = 1; p <= maxPages; p++) {
    const url = p === 1 ? 'https://www.actuarylist.com/' : `https://www.actuarylist.com/?page=${p}`;
    ...
    const pageJobs = await page.evaluate(() => {
      ...
      const cards = Array.from(document.querySelectorAll('div.Job_job-card__YgDAV'));

      return cards.map(card => {
        const company = txt(card.querySelector('p.Job_job-card__company__7T9qY'));
        const title = txt(card.querySelector('p.Job_job-card__position__ic1rc'));
        const locationNodes = card.querySelectorAll('div.Job_job-card__locations__x1exr a');
        const locations = Array.from(locationNodes).map(n => txt(n)).filter(Boolean).join(', ');
        const tags = Array.from(card.querySelectorAll('div.Job_job-card__tags__zfriA a')).map(n => txt(n));
        const meta = txt(card.querySelector('p.Job_job-card__posted-on__NCZaJ'));
    ...
    for (const j of pageJobs) {
      ...
      const tags = Array.from(new Set((j.tags || []).map(t => normalizeText(t)).filter(Boolean))).slice(0, 10);

      if (title && company) {
        results.push({ title, company, location, posting_date, job_type, tags });
      }
    }
  }
  return results;
