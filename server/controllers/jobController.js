...
exports.getJobs = asyncHandler(async (req, res) => {
	...
	res.json({ data, page: pageNum, limit: limitNum, total, totalPages: Math.ceil(total / limitNum) || 1 });
});
...
exports.triggerScrape = asyncHandler(async (req, res) => {
	const token = (req.query.token || req.headers['x-scrape-token'] || '').toString().trim();
	...
	let upserted = 0;
	for (const j of jobs) {
		const key = { title: j.title, company: j.company, posting_date: j.posting_date };
